...
| **DefiLlama (stablecoins)** | `stablecoins.llama.fi/stablecoincharts/Hyperliquid%20L1` | Daily pegged-stablecoin circulating supply on Hyperliquid L1 (USDC is ~95%+ of it) — the float that drives AQAv2 reserve income, a balance/stock metric, not trading volume | `generate_dashboard.py` → live USDC-float chart + live float feeding the AQA estimate card and P/S explorer's AQA presets (dashboard.html only, not written to the DB) | Public, no auth. Wired in 2026-08-17; not per-asset (sums all pegged tokens on the chain) |
...
| **SEC EDGAR (Circle–Coinbase Collaboration Agreement)** | CIK 1876042, Exhibit 10.1 | Primary contract text for the Circle↔Coinbase USDC revenue split (percentages redacted, mechanism confirmed) | Manual document read, not an API | Primary document for the Circle↔Coinbase leg; see the Hyperliquid docs row below for the Coinbase↔Hyperliquid leg |
| **Hyperliquid official docs (AQA)** | `hyperliquid.gitbook.io/hyperliquid-docs/hypercore/aligned-quote-assets` | Primary AQA/AQAv2 mechanism doc: ~90% "cost-adjusted reserve yield" share, 30-day accrual + 8-day settlement to Assistance Fund, HIP-4/validator-perp mandate on a future upgrade, staking/slashing terms for treasury and technical deployers | Manual document read, not an API (no historical/versioned feed) | Located 2026-08-17 — resolves the prior "no primary Hyperliquid document" gap; see `2026-08-17-aqav2-gitbook-confirmation.md` |
| **Crypto news outlets** (CoinDesk, CryptoBriefing, Bankless, JPMorgan research, BlockBeats via secondary reporting) | various | AQA/AQAv2 program details, $6.176B USDC-on-Hyperliquid float, 90% share, Assistance Fund routing, validator-approval counts, accrual dates | Secondary reporting | See `2026-07-24-coinbase-usdc-reserve-income-research.md`, `2026-07-24-usdc-revenue-vs-user-yield-research.md`, and `2026-08-17-aqav2-gitbook-confirmation.md` for full sourcing and caveats |
...
| Live Hyperliquid Foundation/Labs treasury balance or USDC reserve-income *receipts* (i.e. what Coinbase actually pays over) | No public disclosure exists — this is still estimated (float x yield x share). The underlying *float* itself is now live-trackable (see DefiLlama stablecoins row above), which narrows the estimate but doesn't confirm the actual payment amount |
